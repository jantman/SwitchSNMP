<?php
// A PHP5 class to handle some SNMP functions on Cisco Catalyst/IOS switches.
//
  // developed and tested for 4948 : Catalyst 4000 L3 Switch Software (cat4000-I9S-M), Version 12.2(20)EWA4, RELEASE SOFTWARE (fc2)
//
// +----------------------------------------------------------------------+
...
// | $HeadURL:: http://svn.jasonantman.com/rackman/inc/com_jasonantman_#$ |
// +----------------------------------------------------------------------+
...
class com_jasonantman_SwitchSNMP_IOS
{
    ...
    public function getPorts()
	{
	    ...
	    // walk ENTITY-MIB and correlate with IF-MIB
	    snmp_set_oid_numeric_print(TRUE); // we need numeric OIDs
		...
		$myIF['IFMIB-alias'] = snmpget($this->IP, $this->rocommunity, ".1.3.6.1.2.1.31.1.1.1.18.".$idx); // IF-MIB::ifAlias
		...
		// vlan information
		$myIF['VLAN'] = snmpget($this->IP, $this->rocommunity, ".1.3.6.1.4.1.9.9.68.1.2.2.1.2.".$idx); // using CISCO-VLAN-MEMBERSHIP-MIB
		
		// ENTITY-MIB (using indexes from CISCO-STACK-MIB)
		if(isset($EntityMibIndex[$myIF['IFMIB-descr']]))
		  {
		    snmp_set_valueretrieval(SNMP_VALUE_PLAIN);
		    $ENTITY_index = $EntityMibIndex[$myIF['IFMIB-descr']];
		    //fwrite(STDERR, "ifmib descr='".$myIF['IFMIB-descr']." ENTITY_index=$ENTITY_index\n"); // DEBUG
    ...
    public function getPortMACs($vlans)
	{
		...
		$temp = snmprealwalk($this->IP, $this->rocommunity."@".$vlan, ".1.3.6.1.2.1.17.1.4.1.2");
		$dot1dPortIndex = null;
		foreach($temp as $oid => $val)
		  {
		    $Dot1dTOif[(int)str_replace(".1.3.6.1.2.1.17.1.4.1.2.", "", $oid)] = $val;
		    $resultArr[$val] = array(); // create an array to hold the final values
		  }

		// get the MACs from the bridge port index
		$temp = snmprealwalk($this->IP, $this->rocommunity."@".$vlan, ".1.3.6.1.2.1.17.4.3.1.2");
		foreach($temp as $oid => $val)
		  {
		    $if = $Dot1dTOif[$val];
	...
	try
	{
	    $foo = snmpset($this->IP, $this->rocommunity, ".1.3.6.1.4.1.9.9.68.1.2.2.1.2.".$ifIndex, 'i', $vlanNum); // type == 'i' for INTEGER
	}
	catch (Exception $e)
	{
	...
	if($foo != $vlanNum)
	{
	    fwrite(STDERR, "ERROR: snmpset appeared to work, but current value is not the same as the expected value.\nCurrent Value: $foo\nExpected Value: $vlanNum\n");
	    return false;
	}
	return true;
    }
    ...
    private function makeConstants()
	{
	    ...
	    // an array of the fields returned by the getSwitchInfo() method
	    $this->GET_SWITCH_INFO_RETURN_FIELDS = array('sysDescr', 'sysContact', 'sysName', 'sysLocation', 'lastTftpDownload', 'defaultGateway', 'sysUpTime');
	    $this->GET_COMPONENT_INFO_RETURN_FIELDS = array('Descr', 'Firmware', 'Software', 'Serial', 'Model');
